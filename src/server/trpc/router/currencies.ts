import { type Currency, type Prisma, PrismaClient } from "@prisma/client";
import { z } from "zod";
...
export const currenciesRouter = router({
    ...
    createCurrency: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            const lastCurrency = await prisma.currency.findFirst({
                orderBy: {
                    index: "desc",
                },
            });
            const nextIndex = lastCurrency ? lastCurrency.index + 1 : 0;
